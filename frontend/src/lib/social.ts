...
export async function generateSocialPostDraft(params: {
  ...
  try {
    ...
    console.warn("Edge function call error, falling back to client-side direct Pollinations fetch:", error);
  } catch (e) {
    console.warn("Edge function invocation exception, falling back:", e);
  }

  // Dual-redundancy: Direct client-side fetch using Pollinations
  try {
    const fallbackPrompt = `You are an elite social media copywriter. Write a professional high-converting social post for:
Brand Name: ${params.brandName}
Description: ${params.brandDescription || ""}
Goal: ${params.goal || "Brand Awareness"}
Tone: ${params.tone || "confident"}
Topic/Prompt: ${params.prompt || "Write an engaging caption"}
  ...
  "imagePrompt": "visual concepts"
    ...
    if (res.ok) {
      ...
      return {
        ...
        imagePrompt: String(parsed.imagePrompt || parsed.image_prompt || params.prompt || ""),
      };
    }
  } catch (err) {
