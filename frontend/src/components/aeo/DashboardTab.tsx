...
import { Activity, Search, TrendingUp, Cpu, Globe, ArrowUpRight, Loader2 } from "lucide-react";
...
export const DashboardTab = ({ projectId, projectName, domain }: Props) => {
  ...
  // Compute metrics
  const mentionedCount = hasData ? results.filter(r => r.brand_mentioned).length : 0;
  const visibilityScore = hasData ? Math.round((mentionedCount / results.length) * 100) : 84;
  const topPositionMentions = hasData ? results.filter(r => r.brand_mentioned && r.mention_position === 1).length : 143;
  ...
  if (hasData) {
    results.forEach(r => {
      if (r.brand_mentioned) {
        engineCounts[r.model] = (engineCounts[r.model] || 0) + 1;
      }
    });
  }
  
  let primaryEngine = "ChatGPT";
  if (hasData && Object.keys(engineCounts).length > 0) {
    primaryEngine = Object.entries(engineCounts).sort((a, b) => b[1] - a[1])[0][0].toUpperCase();
  }

  const positiveCount = hasData ? results.filter(r => r.mention_sentiment === "positive").length : 0;
  const brandSentimentPct = hasData && mentionedCount > 0 ? Math.round((positiveCount / mentionedCount) * 100) : 92;

  // Visibility Over Time Data
  const visibilityData = hasData ? (() => {
    const days: Record<string, { total: number; mentioned: number }> = {};
    results.forEach(r => {
      const day = new Date(r.scanned_at).toLocaleDateString("en-US", { weekday: 'short' });
      if (!days[day]) days[day] = { total: 0, mentioned: 0 };
      days[day].total += 1;
      if (r.brand_mentioned) days[day].mentioned += 1;
    });
    return Object.entries(days).map(([name, val]) => ({
      name,
      visibility: Math.round((val.mentioned / val.total) * 100)
    }));
  })() : [
  ...
  // Pie Chart Data
  const sourceData = hasData && Object.keys(engineCounts).length > 0 ? Object.entries(engineCounts).map(([name, val]) => ({
    name: name.toUpperCase(),
    value: Math.round((val / mentionedCount) * 100),
    color: MODEL_COLORS[name.toLowerCase()] || MODEL_COLORS.default
  })) : [
  ...
  // Top Queries
  const topQueries = hasData ? results.filter(r => r.brand_mentioned).map(r => ({
    query: r.prompt_text,
    engine: r.model.toUpperCase(),
    sentiment: r.mention_sentiment === "positive" ? "Positive" : r.mention_sentiment === "negative" ? "Negative" : "Neutral",
    position: r.mention_position || 1
  })).slice(0, 10) : [
    ...
    { query: `${projectName || "solospider"} vs competitors`, engine: 'Google AI', sentiment: 'Positive', position: 1 },
    ...
    <div className="space-y-6 mt-6 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground flex items-center mt-1">
              <span className="text-emerald-500 flex items-center mr-1">
                <ArrowUpRight className="h-3 w-3" /> {hasData ? "+8%" : "+12%"}
              </span>
              from last week
            </p>
          </CardContent>
        ...
        <Card className="glass">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{topPositionMentions}</div>
            <p className="text-xs text-muted-foreground flex items-center mt-1">
              <span className="text-emerald-500 flex items-center mr-1">
                <ArrowUpRight className="h-3 w-3" /> {hasData ? "+12" : "+24"}
              </span>
              new mentions
            </p>
          </CardContent>
        ...
        <Card className="glass">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{primaryEngine}</div>
            <p className="text-xs text-muted-foreground mt-1">
              Top citation source for {projectName}
            </p>
          </CardContent>
        </Card>
        <Card className="glass">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{brandSentimentPct}% Positive</div>
            <Progress value={brandSentimentPct} className="h-2 mt-3" />
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="md:col-span-5 glass">
          <CardHeader>
            ...
            <CardDescription className="text-xs font-medium text-ink-2">
              Appearance rate of <span className="font-bold text-primary">{projectName}</span> across AI engines.
            </CardDescription>
          </CardHeader>
