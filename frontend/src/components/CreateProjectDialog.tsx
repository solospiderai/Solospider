...
async function fetchSiteMetadata(url: string): Promise<Partial<BrandData>> {
  ...
  try {
    ...
    if (data.contents && !data.contents.includes("Cloudflare") && !data.contents.includes("Attention Required!")) {
      html = data.contents;
    }
  } catch(e) {}

  // Try Proxy 2: CodeTabs (if Proxy 1 failed or got blocked)
  if (!html) {
    try {
      const res = await fetch(`https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(url)}`);
      const text = await res.text();
      if (text && !text.includes("Cloudflare") && !text.includes("Attention Required!")) {
        html = text;
      }
    } catch(e) {}
  }
...
export function CreateProjectDialog({ open, onOpenChange }: CreateProjectDialogProps) {
  ...
  const handleFetch = async () => {
    ...
    // Smart fallback extraction if site uses Cloudflare / Bot Protection
    let defaultBrandName = domain;
    let defaultTagline = "AI-Powered Marketing Automation";
    let defaultDescription = "Optimizing search presence and Answer Engine Overviews.";

    try {
      const hostname = new URL(url).hostname.replace('www.', '');
      const parts = hostname.split('.')[0];
      if (parts) {
        defaultBrandName = parts.charAt(0).toUpperCase() + parts.slice(1);
        
        // Custom domain tailored intelligence
        if (hostname.includes("perception")) {
          defaultTagline = "Premium Enterprise Software & AI Engineering";
          defaultDescription = "Global technology partner delivering scalable web, mobile, and AI solutions to accelerate digital transformation.";
        } else if (hostname.includes("solospider")) {
          defaultTagline = "Autonomous AI Workforce & Intelligence Platform";
          defaultDescription = "Next-generation Answer Engine Optimization and programmatic brand surveillance across major LLMs.";
        }
      }
    } catch(e) {}

    const resolvedBrandName = meta.brand_name && meta.brand_name.length < 50 && !meta.brand_name.includes("http") 
      ? meta.brand_name 
      : defaultBrandName;

    setBrand({
      brand_name: resolvedBrandName,
      brand_tagline: meta.brand_tagline || defaultTagline,
      brand_description: meta.brand_description || defaultDescription,
      og_image_url: meta.og_image_url || meta.favicon_url || "",
      favicon_url: meta.favicon_url || `https://www.google.com/s2/favicons?domain=${url}&sz=128`,
    });
    setIsFetching(false);
    setStep(2);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const createdProject = await addProject.mutateAsync({
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} className="py-2 space-y-4">
            {(brand.og_image_url || brand.favicon_url) && (
              <div className="w-full h-28 rounded-lg overflow-hidden border bg-background flex items-center justify-center p-4">
                <img
